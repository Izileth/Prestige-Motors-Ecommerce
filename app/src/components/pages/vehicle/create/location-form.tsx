import { motion } from "framer-motion";
import { Button } from "~/src/components/ui/button";
import { VehicleAddressSection } from "~/src/components/pages/vehicle/create/address-section";
import { fadeIn } from "~/src/lib/animations";
import { id } from "date-fns/locale";

interface LocationFormProps {
  onNext: () => void;
  onBack: () => void;
}

export const LocationForm = ({ onNext, onBack }: LocationFormProps) => (
  <motion.div variants={fadeIn} initial="hidden" animate="visible">
    <VehicleAddressSection vehicleId={id.toString()} isEditing />

    <div className="flex justify-between mt-8">
      <Button
        type="button"
        variant="outline"
        onClick={onBack}
        className="border-gray-200 dark:border-gray-800 hover:bg-gray-100 dark:hover:bg-gray-900 text-gray-700 dark:text-gray-300"
      >
        Voltar: Detalhes Técnicos
      </Button>
      <Button
        type="button"
        onClick={onNext}
        className="bg-black hover:bg-gray-900 text-white dark:bg-white dark:text-black dark:hover:bg-gray-100"
      >
        Próximo: Opções Adicionais
      </Button>
    </div>
  </motion.div>
);
